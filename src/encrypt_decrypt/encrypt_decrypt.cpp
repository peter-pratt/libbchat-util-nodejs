...
bchat::array_uc33 extractRecipientPubkeyAsArray(
        const Napi::Object& obj, const std::string identifier) {
...
bchat::array_uc32 extractCommunityPubkeyAsArray(
        const Napi::Object& obj, const std::string identifier) {
...
bchat::array_uc33 extractGroupEd25519PubkeyAsArray(
        const Napi::Object& obj, const std::string identifier) {
...
bchat::array_uc32 extractProBackendPubkeyHex(
        const Napi::Object& obj, const std::string identifier) {
...
bchat::array_uc32 extractEd25519PrivateKeyHex(
        const Napi::Object& obj, const std::string identifier) {
...
Napi::Value MultiEncryptWrapper::multiEncrypt(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        std::vector<unsigned char> random_nonce = bchat::random::random(24);
        ...
        return bchat::encrypt_for_multiple_simple(
...
Napi::Value MultiEncryptWrapper::multiDecryptEd25519(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        return bchat::decrypt_for_multiple_simple_ed25519(
...
Napi::Value MultiEncryptWrapper::attachmentEncrypt(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        bchat::attachment::Domain attachment_domain =
                domain == "attachment" ? bchat::attachment::Domain::ATTACHMENT
                                       : bchat::attachment::Domain::PROFILE_PIC;
        ...
        auto encrypted = bchat::attachment::encrypt(
...
Napi::Value MultiEncryptWrapper::attachmentDecrypt(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        if (decryption_key_bytes.size() != bchat::attachment::ENCRYPT_KEY_SIZE) {
            throw std::invalid_argument("Key size mismatch");
        }

        std::span<const std::byte, bchat::attachment::ENCRYPT_KEY_SIZE> decryption_key_span(
                decryption_key_bytes.data(), bchat::attachment::ENCRYPT_KEY_SIZE);

        auto decrypted = bchat::attachment::decrypt(encrypted_data_bytes, decryption_key_span);
...
Napi::Value MultiEncryptWrapper::encryptFor1o1(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < array.Length(); i++) {
            ...
            ready_to_send[i] = bchat::encode_for_1o1(
...
Napi::Value MultiEncryptWrapper::encryptForCommunityInbox(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < array.Length(); i++) {
            ...
            ready_to_send[i] = bchat::encode_for_community_inbox(
...
Napi::Value MultiEncryptWrapper::encryptForCommunity(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < array.Length(); i++) {
            ...
            ready_to_send[i] = bchat::encode_for_community(
...
Napi::Value MultiEncryptWrapper::encryptForGroup(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < array.Length(); i++) {
            ...
            ready_to_send[i] = bchat::encode_for_group(
                    plaintext,
                    senderEd25519Seed,
                    sentTimestampMs,
                    groupEd25519Pubkey,
    ...
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < first.Length(); i++) {
            ...
            try {
                ...
                decrypted.push_back(bchat::decode_for_community(
    ...
    return wrapResult(info, [&] {
        ...
        for (uint32_t i = 0; i < first.Length(); i++) {
            ...
            try {
                ...
                decrypted.push_back(
                        bchat::decode_envelope(keys, envelopePayload, proBackendPubkeyHex));
