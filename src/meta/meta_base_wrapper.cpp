...
std::unique_ptr<bchat::nodeapi::MetaGroup> MetaBaseWrapper::constructGroupWrapper(
    ...
    return wrapExceptions(info, [&] {
        ...
        auto info = std::make_shared<config::groups::Info>(
                ...
                (dumped_info ? std::make_optional(bchat::to_span(*dumped_info)) : std::nullopt));

        auto members = std::make_shared<config::groups::Members>(
                group_ed25519_pubkey,
                group_ed25519_secretkey,
                (dumped_members ? std::make_optional(bchat::to_span(*dumped_members))
                                : std::nullopt));
        ...
        auto keys = std::make_shared<config::groups::Keys>(
                ...
                group_ed25519_pubkey,
                group_ed25519_secretkey,
                (dumped_keys ? std::make_optional(bchat::to_span(*dumped_keys)) : std::nullopt),
                *info,
                *members);

        return std::make_unique<bchat::nodeapi::MetaGroup>(
