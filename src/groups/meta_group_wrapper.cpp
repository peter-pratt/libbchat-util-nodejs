...
Napi::Object member_to_js(const Napi::Env& env, const member& info, const member::Status& status) {
    ...
    obj["pubkeyHex"] = toJs(env, info.bchat_id);
...
Napi::Value MetaGroupWrapper::metaDump(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        combined.append("info", bchat::to_string(this->meta_group->info->dump()));
        combined.append("keys", bchat::to_string(this->meta_group->keys->dump()));
        combined.append("members", bchat::to_string(this->meta_group->members->dump()));
        auto to_dump = std::move(combined).str();

        return bchat::to_vector(to_dump);
    });
}
...
Napi::Value MetaGroupWrapper::metaMakeDump(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        ...
        // NOTE: the keys have to be in ascii-sorted order:
        combined.append("info", bchat::to_string(this->meta_group->info->make_dump()));
        combined.append("keys", bchat::to_string(this->meta_group->keys->make_dump()));
        combined.append("members", bchat::to_string(this->meta_group->members->make_dump()));
        auto to_dump = std::move(combined).str();
...
Napi::Value MetaGroupWrapper::memberResetAllSendingState(const Napi::CallbackInfo& info) {
    return wrapResult(info, [&] {
        bool changed = false;
        for (auto& member : *this->meta_group->members) {
            auto sending = this->meta_group->members->has_pending_send(member.bchat_id);
            if (sending) {
                this->meta_group->members->set_pending_send(member.bchat_id, false);
                changed = true;
            }
        }
        return changed;
