...
std::vector<unsigned char> toCppBuffer(Napi::Value x, const std::string& identifier) {
    return bchat::to_vector(toCppBufferView(x, std::move(identifier)));
}
...
std::optional<bchat::config::profile_pic> maybeNonemptyProfilePic(
        Napi::Value x, const std::string& identifier) {
    if (x.IsNull() || x.IsUndefined())
        return std::nullopt;
    /**
    ...
    return bchat::config::profile_pic{*url, *key};
...
Napi::Object push_result_to_JS(
        ...
        const bchat::config::Namespace& push_namespace) {
