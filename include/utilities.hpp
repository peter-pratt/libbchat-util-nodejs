...
template <>
struct toJs_impl<bchat::config::Namespace> {
    auto operator()(const Napi::Env& env, bchat::config::Namespace b) const {
...
using push_entry_t = std::tuple<
        bchat::config::seqno_t,
        std::vector<std::vector<unsigned char>>,
        std::vector<std::string>>;

using confirm_pushed_entry_t =
        std::tuple<bchat::config::seqno_t, std::unordered_set<std::string>>;
