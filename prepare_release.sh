...
rm -f ./libbchat_util_nodejs*.tar.gz
python -m venv .venv
. .venv/bin/activate
...
HEADER_PACKAGE_VERSION=$(grep 'LIBBCHAT_NODEJS_VERSION' src/version.h | sed -E 's/.*"([0-9.]+)".*/\1/')
HEADER_GIT_COMMIT=$(grep 'LIBBCHAT_NODEJS_COMMIT' src/version.h | sed -E 's/.*"([A-Za-z0-9.]+)".*/\1/')
...
python3 build_release_archive.py libbchat_util_nodejs-v$PACKAGE_VERSION.tar.gz --include src/version.h

echo "tar archive size:"
du -sh libbchat_util_nodejs*.tar.gz
...
gh release upload v$PACKAGE_VERSION libbchat_util_nodejs-v$PACKAGE_VERSION.tar.gz
echo "GH_RELEASE_URL: $GH_RELEASE_URL"
echo "Update bchat-desktop with this new libbchat-nodejs version with: "
echo "pnpm remove libbchat_util_nodejs; pnpm add https://github.com/peter-pratt/libbchat-util-nodejs/releases/download/v$PACKAGE_VERSION/libbchat_util_nodejs-v$PACKAGE_VERSION.tar.gz"
echo "Done"
